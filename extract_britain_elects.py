...
# if db is dead restart with mongod --dbpath /var/lib/mongodb/ --fork --logpath ~/my-mongod.log

client = MongoClient()
collection = client.twitter.britainelects

wvi_doc_filter = {
    "full_text": {"$regex": re.compile("^Westminster voting intention", re.IGNORECASE)}
}

pm_preference_doc_filter = {
    "full_text": {"$regex": re.compile("Preference for Prime Minister", re.IGNORECASE)}
}
...
pm_preference_docs = collection.find(
    filter=pm_preference_doc_filter, projection=projection
)
